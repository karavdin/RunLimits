...
ele_ifile = ['ele_theta_bdt0p5_chi30_rebinned.root']
muo_ifile = ['../CreateTemplates/mu_theta_bdt0p5_chi30_rebinned.root']
...
#    if not 'ZprimeNarrow' in pname: return False
    ...
    if 'ZprimeWide' in pname: return False
...
#model_summary(model)
execfile('/uscms_data/d2/drberry/ZPrimeSemiLeptonic_2016/RunLimits/MLF_Yields_Uncertainties/utils.py')
...
for p in par_values:
    ...
    elif p == 'dy_rate':      print '%.3f' % 1.50**par_values[p] + ' ' + p
    elif p == 'toptag':       print '%.3f' % 1.25**par_values[p] + ' ' + p
    elif p == 'mistoptag':    print '%.3f' % 1.07**par_values[p] + ' ' + p
    elif p == 'subjbtag':     print '%.3f' % 1.50**par_values[p] + ' ' + p
    elif p == 'qcd_rate':     print '%.3f' % 1.50**par_values[p] + ' ' + p
